...
%	xscale=0.8
	...
	level 3/.style = {rec, align=left, fill=amber!70, text width=40mm, yshift=-15pt, rounded corners=3pt},
	...
	\begin{scope}[every node/.style={level 3}]
		\node [below of = c1, xshift=15mm] (c11) {With random walk};
		\node [below of = c11] (c12) {Without random walk};
		
		\node [below of = c2, xshift=15mm] (c21) {Laplacian Eigenmaps};
		\node [below of = c21, yshift=-10pt] (c22) {Matrix factorization via node approximation};
		
		\node [below of = c3, xshift=15mm] (c31) {Maximizing edge reconstruction probability};
		...
		\node [below of = c4, xshift=15mm] (c41) {Graphlet-based};
		\node [below of = c41] (c42) {Tree pattern-based};
		\node [below of = c42] (c43) {Random walk-based};
		
		\node [below of = c5, xshift=15mm] (c51) {Latent space-based embedding};
		\node [below of = c51, yshift=-10pt] (c52) {Semantic fusion embedding};
	\end{scope}
