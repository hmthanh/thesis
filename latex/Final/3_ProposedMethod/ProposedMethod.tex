...
%	\hline \multicolumn{4}{|r|}{{Tiếp tục ở trang sau}} \\ \hline
	...
	Phân rã ma trận \small{(Matrix factorization)} & Đồ thị toán tử Laplace Eigenmap & \multirow{3}{=}{Xem xét toàn cục các đỉnh lân cận}&
	\multirow{2}{=}{Sử dụng không gian và thời gian tính toán lớn}\\
	\cline{2-2}
	& Phân rã ma trận bằng xấp xỉ đỉnh & & \\
	\hline
	\multirow{3}{=}{Tái cấu trúc cạnh \small{(Edge reconstruction)}} & Cực đại hóa xác suất tái cấu trúc cạnh & \multirow{3}{=}{Huấn luyện tương đối hiệu quả} & \multirow{3}{=}{Tối ưu chỉ sử dụng thông tin cục bộ. Ví dụ như các cạnh (hàng xóm 1 nước) hoặc cặp đỉnh xếp hạng} \\ \cline{2-2}
	& Tối thiểu hóa mất mát dựa trên khoảng cách & & \\ \cline{2-2}
	& Tối thiểu hóa xếp hạng mất mát dựa trên lề & & \\ \hline
	\multirow{3}{=}{Đồ thị lõi \small{(Graph kernel)}} & Dựa trên graphlet & \multirow{3}{=}{Hiệu quả, chỉ tính những nhánh cấu trúc đơn vị mong muốn} & \multirow{3}{=}{Nhánh cấu trúc thì không độc lập. Số chiều nhúng tăng lên theo hàm mũ} \\ \cline{2-2}
	& Dựa trên mẫu nhánh cây & & \\ \cline{2-2}
	& Dựa trên bước nhảy ngẫu nhiên & & \\ \hline
	
	Mô hình sinh \small{(Generative model)} & Nhúng đồ thị dựa trên không gian ẩn & Phép nhúng có thể giải thích được & Khó điều chỉnh lựa chọn phân bố \\ \cline{2-3}
	
	& Nhúng kết hợp ngữ nghĩa & Tận dụng nhiều thông tin nguồn & Yêu cầu một lượng lớn dữ liệu huấn luyện một cách tự nhiên\\
	\hline
	
	Học sâu  \small{(Deep learning)} & Sử dụng bước ngẫu nhiên & \multirow{2}{=}{Hiệu quả và nhanh chóng. Không phải trích đặc trưng} & Chỉ xem xét đến nội dung cục bộ trong một đường đi. Khó để tìm kiếm chiến lược lấy mẫu tối ưu \\ \cline{2-2} \cline{4-4} 
...
\begin{figure}[htbp]
	...
	\begin{subfigure}[b]{0.45\textwidth}
		...
		\caption{Vector embedding TransE ban đầu}
	...
	\begin{subfigure}[b]{0.45\textwidth}
		...
		\caption{TransE embedding sau khi biến đổi}
		\label{fig:TransEEmbeddingAfter}
	\end{subfigure}
	\caption{Mô hình nhúng đồ thị TransE}
