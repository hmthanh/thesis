...
Akrami và cộng sự [2] sử dụng trực giác tương tự như Toutanova et al. [57] để thực hiện phân tích có cấu trúc hơn một chút, vì họ sử dụng nhiều mô hình khác nhau để kiểm tra khoảng cách hiệu suất giữa **FB15k** và **FB15K-237**.

Kadlec et al. [26] chứng minh rằng việc triển khai **DistMult** được điều chỉnh cẩn thận [71] có thể đạt được các màn kết quả hiện đại, vượt qua hầu hết những người kế nhiệm của chính nó, đặt ra câu hỏi về việc chúng tôi đang phát triển các mô hình LP hay chỉ là điều chỉnh siêu tham số-hyperparameters.

Hung Nghiep Tran và cộng sự [59] diễn giải 4 mô hình dựa trên các thành phần ma trận như các trường hợp đặc biệt của cùng một cơ chế tương tác đa nhúng (multi-embedding). Trong công thức của họ, mỗi phần tử k trong KG được biểu thị dưới dạng một tập các vectơ {k (1), k (2), ..., k (n)}; các hàm chấm điểm kết hợp các vectơ như vậy bằng cách sử dụng các sản phẩm trilinear. Các tác giả cũng bao gồm các phân tích và so sánh theo kinh nghiệm giữa các mô hình đã nói và giới thiệu một mô hình đa nhúng mới dựa trên đại số bậc bốn.

Tất cả các phân tích được đề cập ở trên có một phạm vi rất khác nhau từ chúng ta. Mục tiêu chung của họ là để giải quyết các vấn đề cụ thể hoặc điều tra các giả thuyết dọc; mặt khác, mục tiêu của chúng tôi là tiến hành so sánh rộng rãi các mô hình thuộc các họ khác nhau, điều tra các tác động của các lựa chọn thiết kế khác nhau, thảo luận về các phương pháp thực tiễn đo điểm chuẩn khác nhau và nhấn mạnh tầm quan trọng của cấu trúc đồ thị.

Surveys. Niken et al. [45] cung cấp một cái nhìn tổng quan về các kỹ thuật phổ biến nhất trong toàn bộ Học tập quan hệ thống kê **Statistic Relational Learning** trong LP. Các tác giả bao gồm cả các cách tiếp cận truyền thống dựa trên các đặc trưng đồ thị có thể quan sát và các phương pháp gần đây hơn dựa trên các đặc trưng ẩn. Vì bài báo đã được xuất bản, tuy nhiên, rất nhiều tiến bộ hơn nữa đã được thực hiện trong KG Embeddings.

Cai et al. [9] cung cấp một cuộc khảo sát cho toàn bộ trường hợp nhúng đồ thị **Graph Embedding**. Phạm vi của nó không giới hạn ở KG: ngược lại, các mô hình tổng quan xử lý nhiều loại biểu đồ (Đồng nhất, Không đồng nhất, với Thông tin phụ trợ, Được xây dựng từ Dữ liệu không liên quan) với nhiều kỹ thuật khác nhau. Một số mô hình nhúng KG được thảo luận trong phần dành riêng cho các mô hình minimize margin-based ranking loss.

Để kết thúc này, các cuộc khảo sát của [Wang et al. 63](../papers/63_Knowledge_Graph_Embedding_A_Survey_of_Approaches_and_Applications.pdf) và bởi [Dai Quoc Nguyen-40](../papers/40_An_overview_of_embedding_models_of_entities_and_relationships_for_knowledge_base_completion.pdf) có liên quan nhất đến công việc của chúng tôi, vì họ đặc biệt tập trung vào các phương pháp Nhúng KG. Trong công trình của Wang et al. [63], các mô hình được phân nhóm thô sơ dựa trên dữ liệu đầu vào mà chúng dựa vào (chỉ sự kiện; đường dẫn quan hệ; nội dung văn bản; v.v.); các nhóm kết quả trải qua lựa chọn chi tiết hơn nữa, có tính đến bản chất của các chức năng cho điểm của họ (ví dụ: dựa trên khoảng cách hoặc dựa trên ngữ nghĩa). Hơn thế nữa, họ còn mô tả chi tiết cho từng mô hình mà họ bao gồm, nêu rõ đặc thù kiến trúc của nó cũng như sự phức tạp về không gian và thời gian của nó. Cuối cùng, họ tính đến rất nhiều ứng dụng mà các mô hình Nhúng đồ thị tri thức có thể hỗ trợ. Tác phẩm của Nguyen [40] cũng tương tự, mặc dù ngắn gọn hơn và cũng bao gồm các phương pháp hiện đại nhất như **RotatE** [55].
...
## CONCLUSIONS
### KẾT LUẬN
Bài báo đã trình bày phân tích so sánh rộng rãi đầu tiên trên các mô hình LP dựa trên **KG embedding**.

Bài báo đã khảo sát 16 mô hình LP đại diện cho các kỹ thuật và kiến trúc đa dạng, và Bài báo đã phân tích tính hiệu quả và hiệu quả của chúng trên 5 bộ dữ liệu phổ biến nhất trong tài liệu.

Bài báo đã giới thiệu một tập hợp các thuộc tính cấu trúc đặc trưng cho dữ liệu đào tạo và Bài báo đã cho thấy bằng chứng thực nghiệm mạnh mẽ rằng chúng tạo ra các hiệu ứng tối quan trọng trên các dự đoán. Khi làm như vậy, Bài báo đã điều tra các trường hợp cho phép các mô hình thực hiện thỏa đáng, đồng thời xác định các lĩnh vực mà nghiên cứu vẫn còn chỗ để cải tiến.

Bài báo đã thảo luận kỹ lưỡng về thực tiễn đánh giá hiện tại, xác minh rằng họ có thể dựa vào các chính sách-policies cấp thấp khác nhau tạo ra sự không thể so sánh được và trong một số trường hợp kết quả sai lệch. Bài báo đã phân tích các thành phần làm cho các mô hình nhạy cảm nhất với các chính sách này, cung cấp các quan sát hữu ích cho nghiên cứu trong tương lai.